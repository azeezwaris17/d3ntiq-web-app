...
export const DentalMapSVG2: React.FC<DentalMapSVGProps> = ({ selected, onSelect, onClearAll, onGroupSelect }) => {
  ...
  return (
    <div>
      ...
      >
        <defs>
          <linearGradient id="gumGrad2upper" x1="0%" y1="0%" x2="0%" y2="100%">
            <stop offset="0%" stopColor="#f7a8b8" />
            <stop offset="100%" stopColor="#e38194" />
          </linearGradient>
          <linearGradient id="gumGrad2lower" x1="0%" y1="0%" x2="0%" y2="100%">
        ...
        <style>{`
          .tooth2-unit { cursor: pointer; }
          .tooth2-unit:not(.tooth2-selected):hover path { fill: #54D1ED !important; }
          .tooth2-unit.tooth2-selected path { fill: #2563EB !important; }
          .gum2-unit { cursor: pointer; }
          .gum2-unit:not(.gum2-selected):hover rect { fill: #54D1ED !important; }
          .gum2-unit:not(.gum2-selected):hover path { fill: #54D1ED !important; }
        `}</style>

        {/* ── Upper gum ── */}
        <g
          id="gum-upper"
          className={`gum2-unit${isSel('gum-upper') ? ' gum2-selected' : ''}`}
          onClick={() => onSelect('gum-upper')}
          role="button"
          aria-label="Upper Gum"
        >
          <title>Upper Gum</title>
          <rect x={0} y={UPPER_GUM_Y} width={SVG_W} height={GUM_H} fill="transparent" />
          <g transform={`translate(0, ${UPPER_GUM_Y}) scale(${SVG_W / 200}, ${GUM_H / 40}) translate(0, -18)`}>
            <path
              ...
              fill={isSel('gum-upper') ? GUM_SELECTED : 'url(#gumGrad2upper)'}
              stroke={isSel('gum-upper') ? STROKE_SELECTED : 'none'}
              strokeWidth={isSel('gum-upper') ? 1 : 0}
            />
            <circle cx="50" cy="38" r="7" fill="#ffffff" opacity="0.2" />
            <circle cx="100" cy="35" r="8" fill="#ffffff" opacity="0.2" />
            <circle cx="150" cy="38" r="7" fill="#ffffff" opacity="0.2" />
          </g>
        </g>
        ...
        {TOOTH_TYPES.map((type, i) => {
          ...
          return (
            <g
              key={id} id={id}
              className={`tooth2-unit${sel ? ' tooth2-selected' : ''}`}
              onClick={() => onSelect(id)}
              role="button"
        ...
        <g
          id="gum-lower"
          className={`gum2-unit${isSel('gum-lower') ? ' gum2-selected' : ''}`}
          onClick={() => onSelect('gum-lower')}
          role="button"
          aria-label="Lower Gum"
        >
          <title>Lower Gum</title>
          <rect x={0} y={LOWER_GUM_Y} width={SVG_W} height={GUM_H} fill="transparent" />
          <g transform={`translate(0, ${LOWER_GUM_Y}) scale(${SVG_W / 200}, ${GUM_H / 40}) translate(0, 0)`}>
            <path
              ...
              fill={isSel('gum-lower') ? GUM_SELECTED : 'url(#gumGrad2lower)'}
              stroke={isSel('gum-lower') ? STROKE_SELECTED : 'none'}
              strokeWidth={isSel('gum-lower') ? 1 : 0}
            />
            <circle cx="50" cy="18" r="7" fill="#ffffff" opacity="0.15" />
            <circle cx="100" cy="20" r="8" fill="#ffffff" opacity="0.15" />
            <circle cx="150" cy="18" r="7" fill="#ffffff" opacity="0.15" />
          </g>
        </g>
      </svg>

      {/* Quick select buttons — identical to DentalMapSVG */}
